...
public:
    ...
    void setStartPoint(int start_x, int start_y) override;
    void setGoalPoint(int goal_x, int goal_y) override;
    std::vector<int> makePlan() override;
    void loadObstacleInfo(std::shared_ptr<std::vector<bool>> obstacle_info) override;
    std::vector<std::pair<int, int>> makePlanCoordinate();
private:
    double calcGCost(int cell1, int cell2);
